...
public final class ApiUrl {
    ...
    private static final Logger LOG = Logger.getLogger(ApiUrl.class.toString());
    // Base API URL
    ...
    public String generateUrl(final String method, final Map<String, String> params) {
        ...
        key.append(sed);
        final byte[] sha1code = DigestUtils.sha(key.toString());

        String sig = "";
        try {
            sig = encoder(new String(Base64.encodeBase64(sha1code), URL_ENCODING));
        } catch (UnsupportedEncodingException ex) {
            LOG.log(Level.WARNING,"Failed to encode: {0} {1}", ex.getMessage());
        }
    ...
    private static String encoder(final String toEncode) {
        try {
            ...
            LOG.log(Level.WARNING,"Failed to encode: {}", ex.getMessage());
            return "";
        }
    }
